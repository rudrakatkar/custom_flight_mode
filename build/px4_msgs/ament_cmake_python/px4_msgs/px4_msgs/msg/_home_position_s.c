...
bool px4_msgs__msg__home_position__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // valid_attitude
    PyObject * field = PyObject_GetAttrString(_pymsg, "valid_attitude");
    if (!field) {
      return false;
    }
    assert(PyBool_Check(field));
    ros_message->valid_attitude = (Py_True == field);
    Py_DECREF(field);
  }
  {  // manual_home
...
PyObject * px4_msgs__msg__home_position__convert_to_py(void * raw_ros_message)
{
  ...
  {  // valid_attitude
    PyObject * field = NULL;
    field = PyBool_FromLong(ros_message->valid_attitude ? 1 : 0);
    {
      int rc = PyObject_SetAttrString(_pymessage, "valid_attitude", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
