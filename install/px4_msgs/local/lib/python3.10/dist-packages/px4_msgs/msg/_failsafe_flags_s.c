...
bool px4_msgs__msg__failsafe_flags__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // traffic_avoidance_unhealthy
    PyObject * field = PyObject_GetAttrString(_pymsg, "traffic_avoidance_unhealthy");
    if (!field) {
      return false;
    }
    assert(PyBool_Check(field));
    ros_message->traffic_avoidance_unhealthy = (Py_True == field);
    Py_DECREF(field);
  }
  {  // remote_id_unhealthy
...
PyObject * px4_msgs__msg__failsafe_flags__convert_to_py(void * raw_ros_message)
{
  ...
  {  // traffic_avoidance_unhealthy
    PyObject * field = NULL;
    field = PyBool_FromLong(ros_message->traffic_avoidance_unhealthy ? 1 : 0);
    {
      int rc = PyObject_SetAttrString(_pymessage, "traffic_avoidance_unhealthy", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
